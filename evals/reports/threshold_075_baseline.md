# Eval Report: threshold_075_baseline

- date: 2026-04-14T17:04:51Z
- git_commit: unknown
...
stage2:
  ...
  precision@5: 0.2
stage3_runs:
- answer: 畳み込み（CNN）を使わずに画像処理を実現した新しいアーキテクチャとして、Vision Transformer (ViT) や CLIPが挙げられます。これらは画像をパッチ系列に分割し、Transformerを用いて直接処理するアプローチを採用しています。
  citation_file_ids:
  - QzLEo8LTOgGr
  must_mention_coverage: 1.0
  citation_in_ground_truth: 1.0
...
## Pair comparison vs MatchInfo-level segment_location

- baseline: `/eval-data/reports/match_level_seg_loc.json`
- common cases: 11
- only in current: ['012_vocab_mismatch_vit']
...
| Stage 2: segment recall@5 | 0 | 0 | 11 |
| Stage 2: MRR | 0 | 0 | 11 |
